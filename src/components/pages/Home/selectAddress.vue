<template>
  <div>
    ...
    <div class="addAddress" @click="choseAdd()">
        <input type="text" placeholder="请选择省市区" class="txtmangth" disabled="disabled" v-model="userAddress">
    </div>
    <!-- 收货地址三级联动选项 start-->
...
export default {
  ...
  methods: {
    ...
    determine () {
      ...
      this.dataIf.sheng = this.Province
      this.dataIf.shi = this.City
      this.dataIf.qu = this.District
      this.$emit('eventFromChild', this.dataIf)
    },
...
.txtmangth::-webkit-input-placeholder {
  color:  #999999;
  font-size: .28rem;
  font-weight:400;
  line-height:38px;
...
.txtmangth:-ms-input-placeholder {
  color:  #999999;
  font-size: .28rem;
  font-weight:400;
  line-height:38px;
...
.address .title .accolor{
  color: #FF6332;
  border-bottom:0.04rem solid #FF6332;
}
.address ul .active{
  color:#FF6332;
}
.address ul .active span::after{
  content: '';
  background-image: url(../../../assets/image/gou_img.png);
  width: .4rem;
  height: .2rem;
...
.determine{display: inline-block;width: .75rem;text-align: center; float: right;height: .82rem;line-height: .82rem;margin-right: .3rem;color: #FF6332;font-size: .28rem;}
</style>
